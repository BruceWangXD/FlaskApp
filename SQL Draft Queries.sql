...
FROM mediaserver.UserAccount JOIN mediaserver.UserMediaConsumption USING (username)
...
-- Get Album metadata
SELECT md_value
FROM mediaserver.Album LEFT JOIN AlbumMetaData USING (album_id)
                       LEFT JOIN mediaserver.MetaData USING (md_id)
                       LEFT JOIN mediaserver.MetaDataType USING (md_type_id)
WHERE WHERE album_id = '%s' AND md_type_name IN ('artwork', 'description', 'song genre');
          
-- Get data about list of songs --
SELECT song_id, song_title, artist_name
FROM Album JOIN Album_Songs USING (album_id)
           JOIN Song USING (song_id)
           JOIN Song_Artists USING (song_id)
           JOIN Artist ON (performing_artist_id = artist_id)
WHERE album_id = '%s';
ORDER BY track_num;
...
SELECT md_value
FROM Podcast LEFT JOIN PodcastMetaData USING (podcast_id)
             LEFT JOIN MetaData USING (md_id)
             LEFT JOIN MetaDataType USING (md_type_id)
...
FROM mediaserver.Movie JOIN mediaserver.VideoMedia ON (movie_id = media_id)
                       ...
                       LEFT JOIN mediaserver.MetaData USING (md_id)
                       LEFT JOIN mediaserver.MetaDataType USING (md_type_id);WHERE movie_title LIKE '%s%';
